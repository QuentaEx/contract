...
import "../libraries/QuentaPausable.sol";

error QuentaUSD__InvalidOraclePrice();
error QuentaUSD__InvalidToken();
error QuentaUSD__InsufficiantOutputAmount();
error QuentaUSD__InvalidUnderlyingTokenData();

contract QuentaUSD is UUPSUpgradeable, ERC20Upgradeable, QuentaPausable, Delegatable {
    ...
    function setUnderlyingToken(address[] calldata assets, address[] calldata oracles) public onlyOwner {
        if (assets.length != oracles.length) {
            revert QuentaUSD__InvalidUnderlyingTokenData();
        }
        unchecked {
    ...
    function mintUSDFee(address[] calldata tokens) external onlyOwner {
        ...
        for (uint256 i; i < tokens.length; ++i) {
            ...
            if (tokenDecimals == 0) revert QuentaUSD__InvalidToken();
    ...
    function getPrice(address token, bool min) public view returns (uint256 price) {
        ...
        if (oracle == address(0)) {
            revert QuentaUSD__InvalidToken();
        }
        ...
        if (oraclePrice <= 0 || block.timestamp > updatedAt + stalePriceDelay) {
            revert QuentaUSD__InvalidOraclePrice();
        }
    ...
    function buy(address token, uint256 minUsdxOut, uint256 tokenAmount) external whenNotPaused returns (uint256 usdxAmount) {
        ...
        if (usdxAmount < minUsdxOut) {
            revert QuentaUSD__InsufficiantOutputAmount();
        }
